use rocket::{serde::json::Json, http::Status, response, };
use serde::Serialize;
...
impl ErrorResponse {
    pub fn create_404_error(message: &str) -> Error {
      Error::NotFoundError( Json(ErrorResponse {
        message: format!("Status code: {} message: {}", Status::NotFound.to_string(), message),
      }))
    }

    pub fn create_409_error(message: &str) -> Error {
      Error::DuplicateError( Json(ErrorResponse {
        message: format!("Status code: {} message: {}", Status::Conflict.to_string(), message),
      }))
    }

     pub fn create_500_error(message: &str) -> Error {
      Error::InternalError( Json(ErrorResponse {
        message: format!("Status code: {} message: {}", Status::InternalServerError.to_string(), message),
      }))
    }
